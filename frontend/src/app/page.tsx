    ...
    <main className="min-h-screen relative overflow-x-hidden bg-white flex flex-col items-center transition-colors duration-700">
      ...
      <div className="max-w-4xl w-full flex flex-col items-center justify-center pt-4 px-4 z-10 text-center min-h-[80vh]">
        <AnimatePresence mode="wait">
          {!videoData ? (
            <motion.div key={`input-${activeTab}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col items-center w-full">
              <div className="mb-6">{current.icon}</div>
              <h1 className="text-6xl md:text-8xl font-black tracking-tighter mb-10 text-zinc-900 leading-[0.8]">Aura <br/> <span className={`transition-colors duration-500 ${current.color}`}>Summarizer</span></h1>
              <div className="w-full max-w-xl space-y-6">
                <div className="relative group">
                  <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="Cole o link aqui..." className={`w-full py-6 px-10 rounded-full border-2 transition-all bg-white/80 backdrop-blur-md shadow-[0_20px_50px_rgba(0,0,0,0.1)] focus:outline-none text-lg ${error ? 'border-red-500' : 'border-white focus:border-zinc-300'}`} />
            ...
            <motion.div key="result-screen" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center w-full max-w-lg mb-10">
              <div className={`bg-zinc-200 rounded-[3rem] mb-6 overflow-hidden border-[6px] border-white shadow-2xl relative transition-all duration-500 ${isVertical ? 'w-52 aspect-[9/16]' : 'w-full aspect-video md:w-[32rem]'}`}>
                {videoData.thumbnail ? <img src={videoData.thumbnail} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-zinc-400 font-bold uppercase tracking-tighter">Buscando...</div>}
                ...
                {view === "options" ? (
                  <motion.div key="options" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="w-full">
                    <div className="mb-10 w-full px-6">
                      ...
                      {/* LEADING SNUG para não cortar emojis e padding bottom 2 */}
                    ...
                    <div className="flex gap-4 w-full mb-4 px-4">
                      {/* MP4 e MP3 agora com a mesma animação e peso visual */}
                  ...
                  <motion.div key="resolutions" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="w-full px-4">
                    <div className="bg-white p-8 rounded-[3rem] border border-zinc-100 shadow-xl w-full text-left">
                      ...
                      <button className={`w-full py-6 rounded-[2rem] font-black text-white shadow-2xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 ${current.button}`}><Download size={20}/> BAIXAR AGORA</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <AnimatePresence>
                {showAnalysis && view === "options" && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-6 mx-4 p-10 bg-zinc-900 rounded-[3rem] text-left w-full overflow-hidden shadow-2xl">
                    ...
                    <p className="text-zinc-300 font-medium leading-relaxed text-lg italic text-left italic">"{videoData.resumo || "O resumo inteligênte está sendo gerado..."}"</p>
                  </motion.div>
                )}
              </AnimatePresence>
      ...
      <section id="features" className="scroll-mt-20 max-w-6xl w-full px-8 py-24 grid grid-cols-1 md:grid-cols-3 gap-12 border-t border-zinc-100">
        <div className="space-y-4 text-center md:text-left">
          ...
          <h3 className="text-xl font-black text-zinc-800 tracking-tighter uppercase">Resposta Imediata</h3>
          <p className="text-zinc-500 font-medium leading-relaxed text-sm">Arquitetura assíncrona para que você não perca tempo esperando.</p>
        </div>
        <div className="space-y-4 text-center md:text-left">
          ...
          <h3 className="text-xl font-black text-zinc-800 tracking-tighter uppercase">IA Contextual</h3>
          <p className="text-zinc-500 font-medium leading-relaxed text-sm">Entendemos o tom e os pontos cruciais do conteúdo.</p>
        </div>
        <div className="space-y-4 text-center md:text-left">
          ...
          <h3 className="text-xl font-black text-zinc-800 tracking-tighter uppercase">Privacidade Total</h3>
          <p className="text-zinc-500 font-medium leading-relaxed text-sm">Seus dados são limpos periodicamente garantindo segurança.</p>
        </div>
      </section>
      ...
      <footer id="about" className="min-h-screen w-full bg-zinc-900 px-8 flex flex-col items-center justify-center text-center mt-auto py-20">
        <div className="max-w-4xl w-full space-y-16">
          <div className="space-y-6">
            ...
            <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter leading-none">Criado para facilitar <br/> sua rotina digital.</h2>
            <p className="text-base md:text-lg text-zinc-400 font-medium leading-relaxed max-w-2xl mx-auto text-center opacity-70">
              O projeto Aura surgiu da colaboração entre 
              <span className="text-white font-bold mx-1 underline decoration-red-600 underline-offset-4">Vitor Rovani Marcelino</span> e 
              <span className="text-white font-bold mx-1 underline decoration-blue-600 underline-offset-4">Pedro Goulart Branco</span>.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto w-full">
            <motion.div whileHover={{ y: -8, backgroundColor: "rgba(255,255,255,0.06)" }} className="bg-white/5 p-8 rounded-[2.5rem] border border-white/5 text-left transition-all flex flex-col gap-6 group">
              <div className="w-14 h-14 bg-red-600 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-2xl group-hover:rotate-6 transition-transform">VR</div>
              <div>
                <h4 className="font-black text-white text-xl tracking-tight">Vitor Rovani Marcelino</h4>
                <p className="text-red-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2">UX & Frontend Master</p>
              </div>
            </motion.div>
            <motion.div whileHover={{ y: -8, backgroundColor: "rgba(255,255,255,0.06)" }} className="bg-white/5 p-8 rounded-[2.5rem] border border-white/5 text-left transition-all flex flex-col gap-6 group">
              <div className="w-14 h-14 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-2xl group-hover:-rotate-6 transition-transform">PG</div>
              <div>
                <h4 className="font-black text-white text-xl tracking-tight">Pedro Goulart Branco</h4>
                <p className="text-blue-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2">Backend & AI Architect</p>
              </div>
            </motion.div>
          </div>
